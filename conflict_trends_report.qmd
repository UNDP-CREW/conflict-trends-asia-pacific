...
Making sense of this data will require a deeper examination of the various contexts in this region. The gif below displays cumulative conflict events collected by ACLED, over the past 10 years. This shows us how political conflict has been concentrated over the past 10 years, and which populations and regions have been particularly restive. 

Though in no way is this document intended to be an exhaustive analysis of conflict in the region, in our exploratory analysis of the data, we have found the following topics/sections to provide an reasonable overview of conflict trends in the region: 
...
![](./plots/Ethnologue_Map_Main_Spoken_Languages_of_MyanmarNeighbour_MIMU1300v03_7Jan2019_A1_1.png)
...
Judging by the frequency of protest events in countries not at war, pre-covid, the countries and territories with the most dissatisfied populations were Nepal, Hong Kong, Pakistan, Sri Lanka, Cambodia and Bangladesh (with its predominance of riots). Post-covid, populations in Pakistan and Sri Lanka are still voicing their dissatisfaction. For the most part, protests seem to be less lethal after covid.  

Protests have increased in India and Indonesia, though riots are still at the roughly the same levels as before. Maldivians (at the bottom right) are protesting at greater levels than even pre-covid Hong Kong.  
Maldives post-covid what's going on? 

Post-covid, there has been a notable increase in riots across the Asia-Pacific. 

Pakistani protesters had almost as high a fatality rate as protesters in Myanmar. 

Pre-covid, Afghanistan, Pakistan and Myanmar were clearly involved in open warfare, given the high activity and lethality experienced amongst state forces, rebel groups, political militias and civilians (any event involving a combatant and a civilian ends predictably). Bangladeshi and Thai actors border these most prolific actors, indicating significant instability and unrest in those countries. 

Post-covid, we see a rapid increase of deaths in Afghanistan and Myanmar, whereas conflict actors in Pakistan and Philippines were less violent, but still 

Papua New Guinea, where data collection began in `r acled_filtered |> filter(country == "Papua New Guinea")  %>% {min(.$event_date)}`, has showed itself to be one of the 

Pre-covid, it would appear that, just from the chart, Afghanistan, Pakistan, Philippines, India and Myanmar were involved in either civil wars or insurgencies. States with significant violence against civilians and collateral damage (any event involving a combatant actor and a civilian actor ends predictably) were Philippines, Pakistan, Indonesia,  

Before covid, it was Indonesian protesters who died the most. Afghanistan had the most prolific conflict actors prior to and after covid; this is particularly worrisome considering ACLED only began data collection there on `r acled_filtered |> filter(country == "Afghanistan")  %>% {min(.$event_date)}`. Additionally, as unstable that Bangladesh appears in hindsight, Indonesia has also had similar levels of activity amongst rioters and political militias. 

It would appear that during the drug war, 
...
```{r}
acled_filtered |> 
  filter(str_detect(notes, "Meitei")) |> 
  sample_n(5) |> 
  pull(notes)
```
...
![](./plots/protest_network_graph.png)



<br>

A few clusters of protest descriptions: Myanmar, Palestine, Korea and unions, Meitei, Kashmir, citizenship amendment, Fukushima and several around economic concerns (arrears, pensions, retirees, social security)
...
protest_network_graph <- acled_protests |>
  ...
  theme(legend.position = "none", 
        plot.caption = element_text(hjust = .5)) + 
  labs(title = "Network graph of post-covid Indonesian protest descriptions", 
       subtitle = "Line thickness indicates number of events involving those words, line transparency indicates the correlation between words", 
       caption = "Source: www.acleddata.com")
...
protest_country_words |> 
  ...
  mutate(
    word_group = case_when(
      ...
      str_detect(word, "omni|congress|articl|law|legco")  ~ "legislation",
      ...
      str_detect(word, 
                 "military|coup|lese|maj|president|government|monarchy|police|arrest|release|oli") 
      ~ "state", 
...
Place names indicate widespread protests. 


```{r}
acled_filtered |> 
  filter(str_detect(notes, "padlocking") & country == "Nepal") |> 
  sample_n(5) |> 
  pull(notes)
```
...
protest_country_words |> 
  ...
  mutate(
    word_group = case_when(
      str_detect(
        word,
        "worker|arrear|wage|factori|union|kctu|picket|pension|syndic|retire|commod|labor|price|cpi|employer|economic|financial|strike|owe")
      ~ "economic",
      str_detect(word, "tamil|hindu|bjp|nadu|indian") ~ "indian nat.",
      str_detect(word, "civi|disobed") ~ "disobedience",
      str_detect(word, "corona|lockdown|restrict|home|pandem") ~ "covid",
      str_detect(word, "omni|congress|articl")  ~ "legislation",
      str_detect(word, "provin|local|farmer|skm|bku|regen") ~ "rural",
      str_detect(word, "myanmar|kachin|arakan") ~ "myanmar",
      str_detect(word, "kashmir|jammu") ~ "kashmir",
      str_detect(word, "press|editor") ~ "press",
      str_detect(word, "student|university|youth") ~ "student", 
     TRUE ~ "other"),
    word_group = fct_relevel(word_group, 
                             c("economic", "indian", "disobedience", "covid", 
                               "legislation", "rural", "myanmar", "kashmir", 
                               "press", "student", "other"))
  ) |>
  filter(word %out% c("front", "conference", "demanding", 
                      "including", "support", "demand")) |> 
